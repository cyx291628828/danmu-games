...
> 填对 1 格 +scorePerFill 分并在格子上留头像角标；结算面板为领奖台式（整体左对齐）：卡片顶缘居中的👑冠军头像
> 向上突出，第一名行左侧另有大头像，
> 第 2/3 名左侧为头像框（银/铜描边标识名次，不显示名次数字），第 4~6 名仅名字与对/错/分（不显示头像与名次），
> 每人显示填对数、错填数与本局积分；
> 通关时 MVP 额外 +mvpBonus 并计 1 胜。展示屏右栏为<b>数独专属榜</b>：按 MVP 次数（sudoku_wins，通关 MVP 计 1 次）
> 降序排序（同次数按数独总积分 sudoku_score 排序），仅显示 👑MVP 次数列；积分不展示，但后台照常记录。
...
| `giftFillCount` | number | `3` | 送礼触发的随机连填格数 m（每次礼物事件触发一次，m 格同时落定，全部记在送礼观众名下）。上限 81（整盘填满） |
| `followFillCount` | number | `1` | 关注触发的随机填格数 n：观众关注主播时自动随机填 n 个正确数并记在其名下计分（⭐ 徽章），n 格同时落定。`0` = 关注不填格。上限 81。**每位观众每次游戏启动仅奖励一次**（重复关注/取关再关注不再填格，防止刷格；重启服务后重置） |
...
| 日期 | 变更 | 说明 |
|------|------|------|
| 2026-09-07 | 数独新增「关注填n格」助攻（followFillCount，默认 1，0=关闭） | 观众关注主播 → 随机填 n 个正确数并记在其名下计分（格子 ❤ 徽章、操作日志 ❤ 条目）；控制台「互动助攻」组新增配置项；统计芯片/结算统计/历史对局均含关注填数 |
| 2026-09-07 | 数独结算面板改领奖台式分层显示（Top6，整体左对齐） | 卡片顶缘居中的冠军头像向上突出（👑随通关 MVP 显示）；第一名行左侧另有大头像；第 2/3 名左侧为头像框（银/铜描边标识名次，不显示名次数字）；第 4~6 名仅显示名字与对/错/分（不显示头像与名次）；全部名次的名字与对错分左对齐（覆盖 .win-card 的 text-align:center 继承） |
